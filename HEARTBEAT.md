...
<!--
This is NightClaw's replacement for the default OpenClaw HEARTBEAT.md. It is
intentionally the minimum-viable heartbeat: a few quick reads, one consolidation
trigger, and an exit.

The actual operational loop is the two `openclaw cron` triggers set up in
DEPLOY.md (worker + manager). Those are authoritative. The heartbeat is a
