...
| Python / Script ({PLATFORM}) | AVAILABLE | No GUI; write progress to files, not stdout |
| SQLite / DuckDB | AVAILABLE | File-based; DuckDB preferred for flat file analytics |
| Oracle Cloud ERP SQL | UNVERIFIED | Needs Oracle Instant Client + VPN confirmation |
| PostgreSQL Local | UNVERIFIED | Needs local Postgres server confirmed running |
| Python Exec (cron lane) | CONSTRAINED | exec allowlist blocks python3 in cron-event channel. Use interactive session for exec-gated passes. Confirmed paths: python3, /usr/bin/python3 (NOT /usr/local/bin/python3). If your project requires exec in cron, add a PA entry in OPS-PREAPPROVAL.md before your first overnight run. |
...
| Claude Computer Use | UNVERIFIED | Desktop control (mouse/keyboard/screen) launched April 2, 2026; requires Pro/Max plan; verify OAuth $20 plan includes it before use |
...
| Minimum Model Quality | REQUIRED | GPT-5 class minimum (gpt-5.3-codex or equivalent). Below this threshold instruction-following degrades and orchestration controls cannot maintain correctness. Never use gpt-4o or below for cron sessions. |

| scripts/check-lock.py | AVAILABLE (manual only) | No | Deterministic Python lock state checker. Run manually: `python3 scripts/check-lock.py session:nightclaw-worker`. Outputs PROCEED or DEFER. NOT callable from cron sessions (exec allowlist blocks it) — used for manual diagnostics and testing only. |
