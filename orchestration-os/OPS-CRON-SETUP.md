...
> **Model IDs below are illustrative defaults.** NightClaw is model-agnostic.
> Substitute the specific model IDs your OpenClaw install supports, keeping the
> role guidance: worker on a fast, inexpensive execution-class model; manager
> on a more capable judgment-class model.
...
> The full task text lives in `orchestration-os/CRON-WORKER-PROMPT.md`.
...
**Model flag:** none — NightClaw manages the worker model automatically via `MODEL-TIERS.md`. Set initial model before creating the cron: `openclaw models set <id> && openclaw config apply`
...
> The full task text lives in `orchestration-os/CRON-MANAGER-PROMPT.md`.
...
**Model flag:** `--model <manager-model-id>`
**Delivery flag:** `--no-deliver`
...
```
□ Cron 1 created: nightclaw-worker-trigger, every 3h, NO --model flag, session:nightclaw-worker, --light-context
□ MODEL-TIERS.md exists with lightweight/standard/heavy model IDs filled in
□ Platform default model set: openclaw models set <id> && openclaw config apply
□ Cron 2 created: nightclaw-manager-trigger, every 1d, --model <manager-model-id>, session:nightclaw-manager, --light-context
...
# Note: NO --model flag on the worker — NightClaw manages the worker model automatically via MODEL-TIERS.md and set-model-tier.
